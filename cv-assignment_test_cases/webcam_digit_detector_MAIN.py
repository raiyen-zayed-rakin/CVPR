...
def preprocess_digit(roi):
    """Preprocess the detected region for the model - matches notebook preprocessing"""
    ...
    # Reshape to match model input: (1, 28, 28, 1) - CRITICAL FIX
...
while True:
    ...
    for contour in contours:
        ...
        if (min_size < w < max_size and 
            ...
            if roi.size > 0:
                # Preprocess for model - now returns correct shape
                model_input, processed = preprocess_digit(roi)
                
                # Make prediction - model_input is now (1, 28, 28, 1)
                predictions = model.predict(model_input, verbose=0)
                predicted_digit = np.argmax(predictions)
                confidence = predictions[0][predicted_digit]
                
                # Only show predictions with reasonable confidence
                if confidence > 0.7:  # Lowered threshold slightly
                    detections.append({
                        'bbox': (x1, y1, x2, y2),
